...
script_info['optional_options'] = [
 make_option('--collapse_unknonwn',action='store_true',default=False, help="Collapse categories that are described as 'other' or 'unclassified.' The default behavior is to ignore these groups [default: %default]")]
script_info['version'] = __version__

def make_collapse_f(category, level, ignore_unknown):
    def collapse(md):
        ignore_labels = ['unclassified', 'other']
        for path in md[category]:
            if ignore_unknown and path[level].lower() in ignore_labels:
                continue
