...
script_info['optional_options'] = [\
    ...
    make_option('--load_precalc_file_in_biom',default=False,action="store_true",help='Instead of loading the precalculated file in tab-delimited format (with otu ids as row ids and traits as columns) load the data in biom format (with otu as sample_ids and traits as observation_ids) [default: %default]'),
...
def load_data_table(data_table_fp,\
  ...
  transpose=False,verbose=False):
    ...
    if verbose:
        print "Done loading trait table containing %i functions for %i organisms." %(len(genome_table.observation_ids),len(genome_table.sample_ids))
...
def main():
    ...
    if opts.verbose:
        print "Done loading OTU table containing %i samples and %i OTUs." \
          %(len(otu_table.sample_ids),len(otu_table.observation_ids))
    ...
    if opts.verbose:
        print "Loaded %i genes across %i OTUs from gene count table" \
          %(len(genome_table.observation_ids),len(genome_table.sample_ids))
    ...
    if opts.with_confidence:
        ...
        if opts.verbose:
            print "Loaded %i genes across %i OTUs from variance table" \
              %(len(variance_table.observation_ids),len(variance_table.sample_ids))
        ...
        try:
            assert set(variance_table.observation_ids) == set(genome_table.observation_ids) 
        except AssertionError,e:
            for var_id in variance_table.observation_ids:
                if var_id not in genome_table.observation_ids:
        ...
        try:
            assert set(variance_table.sample_ids) == set(genome_table.sample_ids)
        except AssertionError,e:
            for var_id in variance_table.sample_ids:
                if var_id not in genome_table.sample_ids:
                    print "Variance table SampleId %s not in genome_table sample_ids" %var_id
            raise AssertionError("Variance table and genome table contain different OTU ids")

        #sort the observation_ids and sample_ids to be in the same order
        variance_table=variance_table.sortObservationOrder(genome_table.observation_ids)
        variance_table=variance_table.sortSampleOrder(genome_table.sample_ids)
    ...
    if opts.normalize_by_otu:
        ...
        scaling_factors = dict(zip(otu_table.sample_ids,inverse_otu_sums))
